...
def blast(input_query_file,dblocation):
    ...
    command='blastn_menten -db %s -query %s -out %s -outfmt "7 qseqid qlen qstart qend sseqid slen sstart send pident evalue ppos qcovs bitscore gapopen mismatch length ssciname staxid" -evalue 0.001 -num_threads %s'%(dblocation, inputfile, outfile, cores)
    #print (command)
